...
from tstarbot.combat_strategy.army import Army
...
from tstarbot.combat_strategy.renderer import StrategyRenderer
...
Strategy = Enum('Strategy',
                ('RUSH', 'ECONOMY_FIRST', 'ONEWAVE', 'REFORM', 'HARASS'))
...
class ZergStrategyMgr(BaseStrategyMgr):
  ...
  def _init_config(self, dc):
    if hasattr(dc, 'config'):
      if hasattr(dc.config, 'combat_verbose'):
        self._verbose = dc.config.combat_verbose
  ...
  def _organize_army_by_size(self, size):
    ...
    self._create_fixed_size_mutalisk_squads(squad_size=3,
                                            mutalisk_uniform='main_base',
                                            unique=True)
    self._create_fixed_size_mutalisk_squads(squad_size=3,
                                            mutalisk_uniform='sub_base1',
                                            unique=True)
    self._create_fixed_size_mutalisk_squads(squad_size=3,
                                            mutalisk_uniform='sub_base2',
                                            unique=True)
  ...
  def _create_fixed_size_mutalisk_squads(self, squad_size, mutalisk_uniform,
                                         unique=False):
    if unique:
  ...
  def _create_fixed_size_zergling_squads(self, squad_size, uniform,
                                         unique=False):
    if unique:
      squads = [s for s in self._army.squads]
  ...
  def _mutalisk_harass(self, cmd_queue, mutalisk_uniform,
    ...
    rallied_mutalisk_squads = [squad for squad in self._army.squads
                               if squad.uniform == mutalisk_uniform and
                               self._distance(squad.centroid,
                                              harass_station_pos1) < 5]
  ...
  def _get_slope_up_pos(self, base_pos, offset_fac):
    ...
    highest_pos = [pos for pos, h in zip(slope.pos, slope.height) if
                   h == max_height]
