...
from tstarbot.util import sort_units_by_distance
from tstarbot.util import find_first_if
from tstarbot.util import find_n_if
from tstarbot.util import act_worker_harvests_on_target
from tstarbot.util import act_stop
...
class ZergResourceMgr(BaseResourceMgr):
  ...
  def _update_data(self, dc):
    ...
    self.all_bases = \
      collect_units_by_type_alliance(units, UNIT_TYPEID.ZERG_HATCHERY.value) \
      + collect_units_by_type_alliance(units, UNIT_TYPEID.ZERG_LAIR.value)
    self.all_extractors = collect_units_by_type_alliance(
      units, UNIT_TYPEID.ZERG_EXTRACTOR.value)
    self.all_workers = collect_units_by_type_alliance(
      units, UNIT_TYPEID.ZERG_DRONE.value)
    self.all_minerals = collect_units_by_type_alliance(
